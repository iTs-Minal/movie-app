...
import { FaGripLinesVertical, FaImdb } from "react-icons/fa";
...
const Movie = () => {
  ...
  const getRatedMovies = (page: number) => {
    fetch(
      `https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.NEXT_PUBLIC_API_KEY}&page=${page}`
    )
      .then((res) => res.json())
      .then((json) => {
          ...
          <div className="flex flex-wrap mt-8">
            <div className="flex justify-center items-center gap-2 ml-4 my-4">
              ...
              <div className="text-3xl text-semibold font-Ovo text-black dark:text-white">
                <strong>Top ImDB</strong>
              </div>
              <FaImdb className="text-3xl text-semibold text-black dark:text-white" />{" "}
            </div>
